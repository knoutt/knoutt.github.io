...
function pointerPrototype () {
    ...
    this.down = false;
    this.moved = false;
    this.color = { r: 0.15, g: 0.55, b: 0.6 };
}
...
const displayShaderSource = `
    ...
    void main () {
        ...
        float diffuse = clamp(dot(n, l) + 0.78, 0.78, 1.0);
        c *= diffuse;
    #endif
        ...
        float a = clamp(max(c.r, max(c.g, c.b)) * 0.75, 0.0, 1.0);
        a = smoothstep(0.02, 0.55, a);
...
function paletteAt (t) {
    ...
    const c1 = palette[Math.min(index + 1, count - 1)];
    return {
        r: mixChannel(c0[0], c1[0], frac) / 255,
        g: mixChannel(c0[1], c1[1], frac) / 255,
        b: mixChannel(c0[2], c1[2], frac) / 255
    };
}

function colorFromPosition (x, y) {
    const t = (x * 0.7 + y * 0.3) % 1;
    const c = paletteAt(t);
    const lift = 0.08;
    return {
        r: c.r + (1 - c.r) * lift,
...
const mainPointer = pointers[0];

window.addEventListener('pointerdown', e => {
    const pos = getPointerPos(e);
    updatePointerDownData(mainPointer, e.pointerId ?? -1, pos.x, pos.y);
}, { passive: true });
...
window.addEventListener('pointermove', e => {
    ...
    updatePointerMoveData(mainPointer, pos.x, pos.y);
}, { passive: true });

window.addEventListener('pointerup', () => {
    updatePointerUpData(mainPointer);
}, { passive: true });
...
function updatePointerDownData (pointer, id, posX, posY) {
    ...
    pointer.deltaX = 0;
    pointer.deltaY = 0;
    pointer.smoothDeltaX = 0;
    pointer.smoothDeltaY = 0;
    pointer.lastMove = performance.now();
    pointer.color = colorFromPosition(pointer.texcoordX, pointer.texcoordY);
}
...
function updatePointerMoveData (pointer, posX, posY) {
    ...
    const dt = pointer.lastMove ? Math.max(16, now - pointer.lastMove) : 16;
    ...
    const smooth = 0.25;
    pointer.smoothDeltaX += (deltaX - pointer.smoothDeltaX) * smooth;
    pointer.smoothDeltaY += (deltaY - pointer.smoothDeltaY) * smooth;
    pointer.deltaX = pointer.smoothDeltaX;
    pointer.deltaY = pointer.smoothDeltaY;

    const minDelta = 0.0003;
    pointer.moved = Math.hypot(pointer.deltaX, pointer.deltaY) > minDelta;
}
...
function generateColor () {
    ...
    const lift = 0.08;
    return {
        r: clamp01(c.r + (1 - c.r) * lift),
        g: clamp01(c.g + (1 - c.g) * lift),
        b: clamp01(c.b + (1 - c.b) * lift)
    };
}
...
(() => {
  ...
  if (ioTargets.length && 'IntersectionObserver' in window) {
    const io = new IntersectionObserver(
      ...
      { rootMargin: '-50px 0px -50px 0px', threshold: 0.18 }
    );
  ...
  document.querySelectorAll('.project').forEach(project => {
    const main = project.querySelector('.media [data-main]');
    const mediaInner = project.querySelector('.media-inner');
    const desc = project.querySelector('.desc');
    if (!main || !desc) return;

    const defaultImg = main.getAttribute('src');
    const defaultBlurb = desc.dataset.default || desc.textContent.trim();
    let lockedSrc = null;

    function swapImageSmooth(newSrc, pop) {
      if (main.src.endsWith(newSrc)) return;
      ...
      temp.onload = () => {
        ...
        if (pop) {
          ...
          setTimeout(() => main.classList.remove('pop'), 220);
        }
      };
      temp.src = newSrc;
    }

    function setBlurbSmooth(text) {
      desc.classList.add('fade-out');
      setTimeout(() => {
        desc.textContent = text;
        desc.classList.remove('fade-out');
        desc.classList.add('fade-in');
        setTimeout(() => desc.classList.remove('fade-in'), 280);
      }, 140);
    }

    const setLocked = (btn, src) => {
      project.querySelectorAll('[data-thumb]').forEach(b => b.setAttribute('aria-pressed', 'false'));
      if (btn) {
        btn.setAttribute('aria-pressed', 'true');
        lockedSrc = src;
        swapImageSmooth(src, true);
        setBlurbSmooth(btn.dataset.blurb || defaultBlurb);
      } else {
        lockedSrc = null;
        swapImageSmooth(defaultImg, true);
        setBlurbSmooth(defaultBlurb);
      }
    };

    project.querySelectorAll('[data-thumb]').forEach(btn => {
      const img = btn.querySelector('img');
      if (!img) return;
      const src = img.getAttribute('src');

      btn.addEventListener('mouseenter', () => {
        if (!lockedSrc) swapImageSmooth(src);
      });
      btn.addEventListener('mouseleave', () => {
        if (!lockedSrc) swapImageSmooth(defaultImg);
      });

      const toggle = () => {
        if (lockedSrc === src) setLocked(null, null);
        else setLocked(btn, src);
      };
    ...
    if (el && mediaInner && !isMobile && window.matchMedia('(hover: hover)').matches) {
      el.addEventListener('mousemove', e => {
        ...
        const ry = clamp(nx * 6, -6, 6);
        mediaInner.style.setProperty('--tiltX', ry.toFixed(2) + 'deg');
        mediaInner.style.setProperty('--tiltY', rx.toFixed(2) + 'deg');
      });
      el.addEventListener('mouseleave', () => {
        mediaInner.style.setProperty('--tiltX', '0deg');
        mediaInner.style.setProperty('--tiltY', '0deg');
      });
    }
  });
  ...
  const animateLayout = (action, options = {}) => {
    ...
    action();
    const last = new Map();
    tiles.forEach(tile => last.set(tile, tile.getBoundingClientRect()));
    const animations = [];
    tiles.forEach(tile => {
      ...
      const delay = stagger ? Math.min(isMobile ? 50 : 90, distance * (isMobile ? 0.08 : 0.12)) : 0;
      const anim = tile.animate(
        [{ transform: `translate(${dx}px, ${dy}px)` }, { transform: 'translate(0, 0)' }],
        { duration: isMobile ? 360 : 520, easing: 'cubic-bezier(.21,.9,.24,1)', delay, fill: 'both' }
      );
  ...
  const setOpen = tile => {
    tiles.forEach(t => {
      const btn = t.querySelector('.project-hit');
  ...
  const removeSplineLogo = () => {
    ...
    const ensureStyle = root => {
      if (!root || root.__splineLogoStyleAdded) return;
      const style = document.createElement('style');
      style.textContent = '#logo{display:none !important; visibility:hidden !important; pointer-events:none !important;}';
      root.appendChild(style);
      root.__splineLogoStyleAdded = true;
    };
    const ensureObserver = root => {
      if (!root || root.__splineLogoObserver) return;
      const observer = new MutationObserver(() => hideIn(root));
      observer.observe(root, { childList: true, subtree: true });
      root.__splineLogoObserver = observer;
    };
    ...
    const tick = () => {
      ...
      const hidden = hideIn(viewer) || (sr && hideIn(sr));
      if (sr) {
        ensureStyle(sr);
        ensureObserver(sr);
      }
      if (hidden || attempts >= maxAttempts) return;
      requestAnimationFrame(tick);
    };
    tick();
  };
